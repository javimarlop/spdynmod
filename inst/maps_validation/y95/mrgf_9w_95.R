mrgf9b<-function(i){
 ...
 val0<-getValues(mc95_reclass)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i)))))]
 
 val<-val0[!is.na(val0)]
 length(val)->wdata

 valr0<-getValues(mc_res95)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i)))))]
...
fitw9_95<<-sum(unlist(results9),na.rm=T)/length(unlist(results9))
return(fitw9_95)
}
