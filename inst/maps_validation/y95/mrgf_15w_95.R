mrgf15b<-function(i){
 ...
 val0<-getValues(mc95_reclass)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i))))))))]
 val<-val0[!is.na(val0)]
 length(val)->wdata

 valr0<-getValues(mc_res95)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i))))))))]
...
fitw15_95<<-sum(unlist(results15),na.rm=T)/length(unlist(results15))
return(fitw15_95)
}
