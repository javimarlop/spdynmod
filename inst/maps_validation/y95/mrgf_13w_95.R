mrgf13b<-function(i){
 ...
 val0<-getValues(mc95_reclass)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i)))))))]

 val<-val0[!is.na(val0)]
 length(val)->wdata

 valr0<-getValues(mc_res95)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i)))))))]
...
fitw13_95<<-sum(unlist(results13),na.rm=T)/length(unlist(results13))
return(fitw13_95)
}
