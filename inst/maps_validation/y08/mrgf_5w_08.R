mrgf5b<-function(i){
 ...
 val0<-getValues(mc08_reclass)[unique(neigh_cell(neigh_cell(i)))]

 val<-val0[!is.na(val0)]
 length(val)->wdata

 valr0<-getValues(mc_res08)[unique(neigh_cell(neigh_cell(i)))]
...
fitw5_08<<-sum(unlist(results5),na.rm=T)/length(unlist(results5))
return(fitw5_08)
}
