mrgf15b<-function(i){
 ...
 val0<-getValues(mc08_reclass)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i))))))))]
 val<-val0[!is.na(val0)]
 length(val)->wdata

 valr0<-getValues(mc_res08)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i))))))))]
...
fitw15_08<<-sum(unlist(results15),na.rm=T)/length(unlist(results15))
return(fitw15_08)
}
