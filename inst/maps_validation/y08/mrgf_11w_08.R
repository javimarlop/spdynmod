mrgf11b<-function(i){
 ...
 val0<-getValues(mc08_reclass)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i))))))]
 
 val<-val0[!is.na(val0)]
 length(val)->wdata

 valr0<-getValues(mc_res08)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i))))))]
...
fitw11_08<<-sum(unlist(results11),na.rm=T)/length(unlist(results11))
return(fitw11_08)
}
