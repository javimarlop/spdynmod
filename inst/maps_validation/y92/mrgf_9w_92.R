mrgf9b<-function(i){
 ...
 val0<-getValues(mc92_reclass)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i)))))]
 
 val<-val0[!is.na(val0)]
 length(val)->wdata

 valr0<-getValues(mc_res92)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i)))))]
...
fitw9_92<<-sum(unlist(results9),na.rm=T)/length(unlist(results9))
return(fitw9_92)
}
