mrgf15b<-function(i){
 ...
 val0<-getValues(mc92_reclass)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i))))))))]
 val<-val0[!is.na(val0)]
 length(val)->wdata

 valr0<-getValues(mc_res92)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i))))))))]
...
fitw15_92<<-sum(unlist(results15),na.rm=T)/length(unlist(results15))
return(fitw15_92)
}
