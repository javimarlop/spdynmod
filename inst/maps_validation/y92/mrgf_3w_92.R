mrgf3b<-function(i){
 ...
 vi<-getValues(mc92_reclass)[i]
 cats<-getValues(mc92_reclass)[neigh_cell(i)]
 val0<-c(vi,cats)
 val<-val0[!is.na(val0)]
 length(val)->wdata

 vir<-getValues(mc_res92)[i]
 catsr<-getValues(mc_res92)[neigh_cell(i)]
 valr0<-c(vir,catsr)
...
fitw3_92<<-sum(unlist(results3),na.rm=T)/length(unlist(results3))
return(fitw3_92)
}
