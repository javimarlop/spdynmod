mrgf3b<-function(i){
 ...
 vi<-getValues(mc97_reclass)[i]
 cats<-getValues(mc97_reclass)[neigh_cell(i)]
 val0<-c(vi,cats)
 val<-val0[!is.na(val0)]
 length(val)->wdata

 vir<-getValues(mc_res97)[i]
 catsr<-getValues(mc_res97)[neigh_cell(i)]
 valr0<-c(vir,catsr)
...
fitw3_97<<-sum(unlist(results3),na.rm=T)/length(unlist(results3))
return(fitw3_97)
}
