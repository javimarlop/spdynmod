mrgf5b<-function(i){
 ...
 val0<-getValues(mc97_reclass)[unique(neigh_cell(neigh_cell(i)))]

 val<-val0[!is.na(val0)]
 length(val)->wdata

 valr0<-getValues(mc_res97)[unique(neigh_cell(neigh_cell(i)))]
...
fitw5_97<<-sum(unlist(results5),na.rm=T)/length(unlist(results5))
return(fitw5_97)
}
