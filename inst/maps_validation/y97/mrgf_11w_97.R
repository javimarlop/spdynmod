mrgf11b<-function(i){
 ...
 val0<-getValues(mc97_reclass)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i))))))]
 
 val<-val0[!is.na(val0)]
 length(val)->wdata

 valr0<-getValues(mc_res97)[unique(neigh_cell(neigh_cell(neigh_cell(neigh_cell(neigh_cell(i))))))]
...
fitw11_97<<-sum(unlist(results11),na.rm=T)/length(unlist(results11))
return(fitw11_97)
}
